...
//Список цветов
const gemsColors = ["blue", "green", "red", "yellow"];
...
export const GamePage = () => {
  const [currentColorArrangement, setCurrentColorArrangement] = useState<
    colorsType[]
  >([]);
  const [squareBeingDragged, setSquareBeingDrugged] =
    useState<null | SquareType>(null);
  ...
  const setGemsColor = useCallback(
    (color: colorsType, count: number) => {
      if (color === "blue") {
  ...
  const checkForColumnOfFour = useCallback(() => {
    for (let i = 0; i <= 39; i++) {
      const columnOfFour = [
        i,
        i + CONSTANTS.SCENE_WIDTH,
        i + CONSTANTS.SCENE_WIDTH * 2,
        i + CONSTANTS.SCENE_WIDTH * 3,
      ];
  ...
  const moveIntoSquareBelow = useCallback(() => {
    for (let i = 0; i <= 55; i++) {
      ...
      if (currentColorArrangement[i + CONSTANTS.SCENE_WIDTH] === "") {
        currentColorArrangement[i + CONSTANTS.SCENE_WIDTH] =
    ...
    if (squareBeingDragged === null) {
      ...
      if (validMovesCheck(squareBeingDragged.id, e?.target?.id)) {
        ...
        currentColorArrangement[squareBeingReplacedId] =
          squareBeingDragged.color as colorsType;
        currentColorArrangement[squareBeingDraggedId] = e?.target
          ?.title as colorsType;
        ...
        if (
          ...
          currentColorArrangement[squareBeingReplacedId] = e?.target
            ?.title as colorsType;
  ...
  return (
    <GameField>
      ...
      <SecondColumn>
        <Sound isMuted={isMuted} setMuted={setIsMuted}/>
        <Modal />
        <Time />
